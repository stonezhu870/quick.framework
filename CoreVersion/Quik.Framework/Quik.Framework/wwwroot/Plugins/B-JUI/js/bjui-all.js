...
+function ($) {
        ...
        var tools = {
            ...
            setOp: function(op) {
                if (!op.name) {
                    ...
                    op.quicksort   = (typeof op.quicksort   === 'undefined') ? false  : op.quicksort
    ...
    Datagrid.prototype.refresh = function(filterFlag) {
        ...
        //清除选中行
        var rows = that.$element.data('selectedDatas');
        if (rows !== undefined && rows !== null) {
            if (rows.length > 0) {
                for (var i = 0; i < rows.length; i++) {
                    that.$element.datagrid('selectedRows', rows[i].gridIndex, false);
                }
            }
